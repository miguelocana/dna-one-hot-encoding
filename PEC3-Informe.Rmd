...
```{r}
onehot <- function (df, col_seq, onehot_cols_name='H', keep_seq = FALSE) {
...
# Recoge la seq/string de cada fila y las divide en vectores
for (j in 1:nrow(df2)) {
...
# Entra la seq para ser traducida
seq_onehot <- list()


df2 <- df[,c(x[-grep(col_seq, colnames(df))])] # ELIMINA LA COL_SEQ
