...
Siguiendo la idea de lo propuesto para esta PEC (más allá de hacer un informe con los resultados), se ha desarrollado un informe dinámico con ```Shiny```, de tal manera que cualquier usuario pueda indagar sobre cualquier otra colección de promotores. En este informe dinámico se reutiliza el código que se ha utilizado igualmente para hacer este trabajo. 

Se puede acceder al mismo a través del siguiente link: 

1. La clase o variable dependiente debe situarse en la primera columna.
2. La secuencia de ADN debe situarse en la última columna.

Paralelamente, se ha realizado la PEC siguiendo el workflow habitual para esta asignatura.
...
Primero, se importan los datos y se estudian para ver que tratamientos necesitan.
```{r}
...
Observaciones de los árboles de clasificación:
...
test_labels <- split_train_test(df)$test[,1]
tree_m <- C5.0(train, train_labels)
summary(tree_m)
```

¿Cómo se intepretan los resultados?
- La posición 57 de la codificación one-hot tiene una importancia del 100% en la creación del árbol de decisión.
- El porcentaje de error durante el entrenamiento ha sido ```2.7```.
